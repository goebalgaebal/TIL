...
Day 1 - 5 : 파이썬/ 기초통계 → DB, 서버 환경, 파이썬 연동, 파이썬 기본

Day 6 : 파이썬 데이터 시각화 → matplotlib, 컴퓨터비전(영상처리) → 딥러닝

Day11 : 머신러닝 라이브러리 → 컴퓨터비전을 위한 Library

---

##### 파이썬 for Beginner P.351
...
2. 이진파일
   ...
   * 이미지 파일
     ...
     * RGB(A = 투명도)의 값(각각 0 ~ 255 = 0 ~ FF = 8bit = unsigned 1byte)을 가진다

       → 4byte(각 1byte) = true color

     * 해상도 = 픽셀의 개수

     * 512 * 512(해상도) = 256 KB * 4byte(색) = 1MB

     * Gray scale 

       한 픽셀을 표현하기 위해 1byte(R, G, B 값이 동일)를 사용하는 것

---

##### 파이썬 for Beginner P.377

### 객체지향

현실 세계를 그대로 프로그래밍으로 구현

절차적 → 구조적(모듈-함수화) → 객체지향(구조적 + 공학적 개념)

요구되는 시스템의 규모가 커지면서 구조적 프로그래밍으로 구현할 수 있지만 소요시간 증가 → 객체지향 프로그래밍 등장

* 클래스 class = 설계도, 프로그램의 실체 △, 개념에 가깝다

  * 객체의 속성 = class 내의 변수

  * 객체의 기능(행위) = class 내의 함수 = **메소드**

    sorted() = 함수/ list.sort() = 메소드
...
##### 파이썬 for Beginner P.353

canvas에는 점을 그릴 수 없다

canvas에 paper(빈 이미지)를 올려서 이미지를 처리

```python
#Code06-02 컴퓨터비전 01.py
from tkinter import *
...
1. [화소점 처리]에 다음 메뉴를 추가하고, 구현
   ...
   6. 이진화(=흑백 영상)
   7. 입력/ 출력 영상의 평균값 - 출력은 messagebox
...
![1560323691388](./20190612/1560323691388.png)

### Lookup Table

연산량을 줄이기 위해 미리 계산값을 table로 만들어 놓는 것

[참고](<https://m.blog.naver.com/laizenti/220051962705>)
