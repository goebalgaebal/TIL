...
import sqlite3
import csv # saveToCsv(), loadFromCsv()
...
def saveSqlite() :
    ...
    sql += "raw_id INT AUTO_INCREMENT PRIMARY KEY, raw_fname VARCHAR(30), raw_extname CHAR(5), "
    ...
    avgVal, maxVal, minVal = findStat(fullname)  # 평균, 최대, 최소
    print(fname, extname, fsize, height, width, avgVal, maxVal, minVal)
...
if __name__ == '__main__':
    ...
    mainMenu.add_cascade(label="CSV처리", menu=comVisionMenu6)
    comVisionMenu6.add_command(label="CSV 저장", command=saveToCsv)
    comVisionMenu6.add_command(label="CSV 열기", command=loadFromCsv)
